...
function normalizeImageName(imageName, fallbackExtension) {
  const baseName = String(imageName || "damage-proof")
    .trim()
    .replace(/\.[a-zA-Z0-9]+$/, "")
    .replace(/[^a-zA-Z0-9._-]/g, "-")
    .slice(0, 80);

  return `${baseName || "damage-proof"}.${fallbackExtension}`;
}
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    const notificationContext = await getCustomerContextByThread(req.params.threadId);

    if (notificationContext?.customer_id) {
      const vehicleLabel = [
        notificationContext.brand,
        notificationContext.model,
        notificationContext.registration_number,
      ]
        .filter(Boolean)
        .join(" ");
      ...
      await insertCustomerNotification({
        ...
        serviceRecordId: notificationContext.service_record_id,
        chatThreadId: notificationContext.chat_thread_id,
        sourceType: "chat",
        actionType: "message_received",
        title: "New message from mechanic",
        message: [
          `${currentUser.name || "Workshop team"} sent you a new message`,
          vehicleLabel ? `about ${vehicleLabel}.` : "about your vehicle.",
          messageText ? `Message: ${messageText.slice(0, 140)}${messageText.length > 140 ? "..." : ""}` : "Open chat to view the latest update.",
        ]
          .filter(Boolean)
          .join(" "),
      });
